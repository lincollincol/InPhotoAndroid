...
import com.linc.inphoto.databinding.ItemImageMessageIncomingBinding
...
class InImageMessageItem(
    private val messageUiState: MessageUiState
) : BindableItem<ItemImageMessageIncomingBinding>(messageUiState.getStateItemId()) {
    override fun bind(viewBinding: ItemImageMessageIncomingBinding, position: Int) {
        with(viewBinding) {
            fileImageView.apply {
                loadImage(
    ...
    override fun getLayout(): Int = R.layout.item_image_message_incoming

    override fun initializeViewBinding(view: View) = ItemImageMessageIncomingBinding.bind(view)
}
...
import com.linc.inphoto.databinding.ItemVideoMessageIncomingBinding
...
class InVideoMessageItem(
    private val messageUiState: MessageUiState
) : BindableItem<ItemVideoMessageIncomingBinding>(messageUiState.getStateItemId()) {
    override fun bind(viewBinding: ItemVideoMessageIncomingBinding, position: Int) {
        with(viewBinding) {
            fileImageView.apply {
                loadImage(
    ...
    override fun getLayout(): Int = R.layout.item_video_message_incoming

    override fun initializeViewBinding(view: View) = ItemVideoMessageIncomingBinding.bind(view)
}